...
- The installer runs shell in a target repo, and the curl one-liner is `curl | bash`
  (trust the source and the pinned ref).
- `apply-regions.sh` and `check-markers.sh` parse files from the target repo with
  `awk`/`git` — treat their input as untrusted.
- Cutover and migration do `git mv`/archival across the tree — reversible via
  `legacy/`, but destructive-shaped; stage by name and review.

### Lessons
- **2026-09-16** — Agent descriptions inject into *every* session's tool roster
  independent of `CLAUDE.md`. A retired agent left in `.claude/agents/` will
  self-route sessions back into the dead pipeline (the v1 `engineering-manager`
  "use PROACTIVELY" trap). Retiring an agent means moving it *out* of
  `.claude/agents/`, not just rewriting the docs.
- Seeded/project values must live in a `keep` region or a `once` file — anything
  in a harness-owned region is overwritten on update.
- Keep the core lean: machinery that creates merge conflicts on high-value files
  gets bypassed, and then the harness is just files with a `.toml` next to them.
